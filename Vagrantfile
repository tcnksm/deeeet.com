...
cloud_config_db_path = File.join(File.dirname(__FILE__), "terraform/cloud-config-db.yml")
...
cloud_config_db_path= set_new_discovery(cloud_config_db_path,discovery)
...
Vagrant.configure("2") do |config|
  ...
  config.vm.define vm_name = "deeeet-com-db" do |config|
    config.vm.hostname = vm_name
    ...
    config.vm.network :private_network, ip: "172.20.20.103"

    # Place cloud-config on CoreOS
    config.vm.provision :file, :source => "#{cloud_config_db_path}", :destination => "/tmp/vagrantfile-user-data"
    config.vm.provision :shell, :inline => "mv /tmp/vagrantfile-user-data /var/lib/coreos-vagrant/", :privileged => true
  end
